   ...
   <script>
   	$(function(){
		...
		callbacks: { //여기 부분이 이미지를 첨부하는 부분
			...
			//하단 함수 호출
			uploadImage(files[0],this); //이미지 업로드 호출 - 이미지파 & 현재위치값
			  },
			  onPaste: function (e) {
   	...
   	function uploadImage(file,this_location){ //매개변수 - file:이미지파일 / this_location:위치값
   		var f_data = new FormData(); //ajax에서 전송할 때 form형태로 전송
   			f_data.append("file",file); //file이미지 첨부
   			
   		$.ajax({
   			url:"/uploadImage",
   			type:"post",
   			dataType:"text",  //파일을 받는 형태 - text,json,xml...
   			data:f_data,
   			contentType:false, //파일을 보내는 형태 / 안 적을 시,default값 : application/x-wwww.-form-urlencoded(어플리케이션 form 형태로 보냄)
   			//contentType에 false 형태로 하지 않을 경우, 이미지 파일첨부가 불가능.
   			processData:false, //query String 형태로 보내지 않음. - http://localhsot:8000/bno=1&
   			cashe:false, //남아 있는 데이터 정보를 새롭게 정리(삭제)
   			enctype:"multipart/form-data",
   			success:function(data){ //서버에 가서 이미지를 저장 후 파일 이름을 리턴
   				// data : /upload/000000.1jpg
