...
<html>
	...
	<body>
			...
			<c:if test="${list.size()!=0 }">
				<c:forEach var="bdto" items="${list}" >
					<tr>
					  <td>${bdto.bno}</td>
					  <td>
					  <c:forEach begin="1" end="${bdto.bindent}" step="1">
					   ▶
					  </c:forEach>
						  <a href="bview?bno=${bdto.bno}">
						  	${bdto.btitle}</a> <c:if test="${bdto.bfile!=null }">
		...
		<c:if test="${page!=1}">
		  <a href="blist?page=1"><span>&#171;</span></a>
		</c:if>
		<!-- 이전페이지 이동 -->
		<c:if test="${page>1}">
		...
		<div>
		  <a href="write"><button type="button" style="cursor:pointer;">글쓰기</button></a>
